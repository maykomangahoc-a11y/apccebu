        ...
        async function loadDashboardData() {
            try {
                ...
                dispatchPlanData = (await dispatchRes.json().catch(() => [])).filter(item => item.archiveStatus !== 'Archived' && item.type !== 'STN' && item.type !== 'DD');
            ...
            fetch(`${API_BASE_URL}/outbound-bl-data`)
                .then(r => r.json())
        ...
        function updateDashboard() {
            ...
            const upcomingStats = getStatusBreakdown("Upcoming Plan");
            ...
            assignStat('status-upcoming-plan-qty', upcomingStats.planQty);
            assignText('status-upcoming-plan-count', `${upcomingStats.planCount} orders`);
            assignStat('status-upcoming-picked-qty', upcomingStats.pickedQty);
            assignText('status-upcoming-picked-count', `${upcomingStats.pickedCount} orders`);
            assignStat('status-upcoming-pending-qty', upcomingStats.pendingQty);
            assignText('status-upcoming-pending-count', `${upcomingStats.pendingCount} orders`);
